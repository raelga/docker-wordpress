...
function envo_magazine_customizer_theme_info( $wp_customize ) {
    ...
    $wp_customize->add_section( 'theme_info', array(
  		'title'       => esc_html__( 'Video Documentation & Demo' , 'envo-magazine' ),
  		'priority'    => 6,
  	) );
 ...
 */
class envo_magazine_Customize_Section_Pro extends WP_Customize_Section {
...
add_action( 'customize_register', 'envo_magazine_page_sections_pro' );
function envo_magazine_page_sections_pro( $manager ) {
	// Register custom section types.
	$manager->register_section_type( 'envo_magazine_Customize_Section_Pro' );

	// Register sections.
	$manager->add_section(
		new envo_magazine_Customize_Section_Pro(
			$manager,
			'envo_magazine_page_view_pro',
			array(
				'title'    => esc_html__( 'PRO version', 'envo-magazine' ),
				'priority' => 5, 
				'pro_text' => esc_html__( 'View PRO version', 'envo-magazine' ),
				'pro_url'  => 'https://envothemes.com/product/envo-magazine-pro/'
			)
		)
	);
}